...
## Phase 0 — Baseline (you are here, ~70% done)
...
- Agent v0: LangGraph, tools = `search_historic_resolutions(thread)` (over Gold), `escalate(reason)`; prompts reuse your `4.prompt_engineering` techniques (start: few-shot + instruction-based, temp 0)
...
- Metrics: intent accuracy/F1, escalation precision/recall, reply: BLEU/ROUGE (auto) + 1-5 judge rubric (groundedness, tone-match, actionability, no-hallucination)
